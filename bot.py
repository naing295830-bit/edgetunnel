...
ADMIN_IDS = ["7289768738", "7289768738"]
FORWARD_CHANNEL = "@MYANMARVIPMOD"
    ...
    if call.data == "menu_free_trial":
        if user_id not in paid_users and user_id not in approve:
            await bot.edit_message_text(
                chat_id=chat_id,
                message_id=call.message.message_id,
                text="❌ သင်၏ user ID ကို registered မလုပ်ရသေးပါ။\n\nPAID USER ဖြစ်ရန် Admin @Developer12920 သို့ ဆက်သွယ်ပါ။",
...
async def handle_key(message):
    ...
    if not check_key_expiration(key_info["expires_at"]):
        await bot.reply_to(message, "❌ သင်၏ KEY Expired ဖြစ်နေပါသည်။ ကျေးဇူးပြု၍ Admin @Developer12920 သို့ ဆက်သွယ်ပါ။")
        return
