...
# ==================== CONFIGURATION ====================
BOT_TOKEN = os.environ.get("BOT_TOKEN", "8703792367:AAFyZY0UzXULqv-Gwp5GgftQixggzFF-aYs")
GITHUB_TOKEN = os.environ.get("GITHUB_TOKEN", "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")
...
ဥပမာ:
/portal https://portal-as.ruijienetworks.com/api/auth/wifidog?stage=portal&gw_id=c8cd5548c6dd&gw_sn=H1T0773002302&gw_address=192.168.110.1&gw_port=2060&ip=192.168.110.55&mac=ec:46:2c:db:e8:3b&slot_num=16&nasip=192.168.1.160&ssid=VLAN233&ustate=0&mac_req=1&url=http%3A%2F%2Fconnectivitycheck%2Egstatic%2Ecom%2Fgenerate%5F204&chap_id=%5C277&chap_challenge=%5C115%5C346%5C001%5C325%5C136%5C072%5C016%5C325%5C164%5C101%5C064%5C340%5C354%5C263%5C137%5C357
...
async def handle_portal(message):
    ...
    if await check_session_url_improved(session_url=url):
        ...
        await bot.reply_to(
            ...
            f"`https://portal-as.ruijienetworks.com/api/auth/wifidog?stage=portal&gw_id=c8cd5548c6dd&gw_sn=H1T0773002302&gw_address=192.168.110.1&gw_port=2060&ip=192.168.110.55&mac=ec:46:2c:db:e8:3b&slot_num=16&nasip=192.168.1.160&ssid=VLAN233&ustate=0&mac_req=1&url=http%3A%2F%2Fconnectivitycheck%2Egstatic%2Ecom%2Fgenerate%5F204&chap_id=%5C277&chap_challenge=%5C115%5C346%5C001%5C325%5C136%5C072%5C016%5C325%5C164%5C101%5C064%5C340%5C354%5C263%5C137%5C357`",
            parse_mode="Markdown"
        )
...
async def get_captcha(chat_id, req_session, session_url):
    ...
    async with entry["lock"]:
        if entry["session_id"] and entry["auth_code"]:
            return entry["session_id"], entry["auth_code"]
        session_id = await get_session_id(req_session, session_url, entry.get("session_id"))
        if not session_id:
            return None, None
        for _ in range(10):
            image = await Captcha_Image(req_session, session_id)
            text = await Captcha_Text(image)
            verified = await Varify_Captcha(req_session, session_id, text)
            if verified:
...
async def get_session_id(req_session, session_url, previous_session_id=None):
    mac = get_mac()
    ...
    try:
        async with req_session.get(session_url, headers=headers, allow_redirects=True) as req:
            ...
            async with session.get(url, headers=headers) as req:
                if req.status == 200:
                    ...
                    if respond.get('success'):
                        ...
                        return f"📋 Plan: {profile_name} | ⏳ Time: {totaltime}", raw_minutes
        except Exception as e:
...
async def Captcha_Image(req_session, session_id):
    headers = {
        'authority': 'portal-as.ruijienetworks.com',
    ...
    async with req_session.get('https://portal-as.ruijienetworks.com/api/auth/captcha/image', params=params, headers=headers) as req:
        return await req.read()

async def Varify_Captcha(req_session, session_id, text):
    headers = {
        'authority': 'portal-as.ruijienetworks.com',
        'accept': '*/*',
        'content-type': 'application/json',
    ...
    async with req_session.post('https://portal-as.ruijienetworks.com/api/auth/captcha/verify', headers=headers, json=json_data) as req:
        data = await req.json()
